...
indel_mutation_file='/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_indel_list.annovar.variant_function'
...
colnames(indel_mutation) <- c("region", "gene", "chr", "start", "end", "ref", "alt", "donor", "single_cell_ID")
indel_mutation = indel_mutation %>% separate(donor, c('donor', 'batch'))

indel_mutation$gene <- str_remove(indel_mutation$gene, "\\(.*\\)$") # remove mutations corresponding to more than one genes
indel_mutation <- indel_mutation[!str_detect(indel_mutation$gene, ","), ]
indel_mutation_meta = dplyr::left_join(indel_mutation, metadata, by=c("single_cell_ID" = "sample"), suffix = c(".x", ""),)
indel_mutation_tau = indel_mutation_meta[indel_mutation_meta$group=='Tau',]
indel_mutation_notau = indel_mutation_meta[indel_mutation_meta$group=='noTau',]
...
Tau_exonintron_ready=gene_length_deduped$gene %in% indel_mutation_tau$gene[indel_mutation_tau$region %in% c("exonic","intronic")]
names(Tau_exonintron_ready)=gene_length_deduped$gene
Tau_raw=my.GOseq(Tau_exonintron_ready,gene_length_deduped$merged,"/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_Tau.exonic_intronic.GO.tsv",FALSE)

noTau_exonintron_ready=gene_length_deduped$gene %in% indel_mutation_notau$gene[indel_mutation_notau$region %in% c("exonic","intronic")]
names(noTau_exonintron_ready)=gene_length_deduped$gene
noTau_raw=my.GOseq(noTau_exonintron_ready,gene_length_deduped$merged,"/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_noTau.exonic_intronic.GO.tsv",FALSE)
...
write.table(idx_diff_permutation,file="/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_Tau-noTau.exonic_intronic.raw.tsv",quote=F,sep="\t",row.names=F)

idx_diff_permutation=read.delim("/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_Tau-noTau.exonic_intronic.raw.tsv",header=T)
go_table=data.frame(go_list,
					sapply(go_list,function(x){sum(indel_mutation_tau$gene[which(indel_mutation_tau$region %in% c("exonic","intronic"))] %in% names(gene_map)[grep(x,gene_map)],na.rm=T)}),
					sapply(go_list,function(x){sum(indel_mutation_notau$gene[which(indel_mutation_notau$region %in% c("exonic","intronic"))] %in% names(gene_map)[grep(x,gene_map)],na.rm=T)}),
					sum(indel_mutation_tau$region %in% c("exonic","intronic"),na.rm=T),
					sum(indel_mutation_notau$region %in% c("exonic","intronic"),na.rm=T),
...
write.table(go_table,file="/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_Tau_vs_noTau.exonic_intronic.GO.tsv",quote=F,sep="\t",row.names=F)


go_table = read.delim("/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_Tau_vs_noTau.exonic_intronic.GO.tsv", header=T)
...
pdf("/n/data1/bwh/pathology/miller/lab/AD_Tau/results_hg19/annovar/indel_Tau_vs_noTau.exonic_intronic.abs.pdf",height=5,width=8)
