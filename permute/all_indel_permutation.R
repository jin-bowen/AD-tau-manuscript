...
average_mut_num_summary_ID4 <- AD_average_mut_num_rescale %>% group_by(decile) %>% summarise("average_enrichment" = mean(ID4_enrichment),
                                                                                        "sd_enrichment" = sd(ID4_enrichment))

average_mut_num_summary_ID22 <- AD_average_mut_num_rescale %>% group_by(decile) %>% summarise("average_enrichment" = mean(ID22_enrichment),
                                                                                        "sd_enrichment" = sd(ID22_enrichment))


p <- ggplot(average_mut_num_summary_ID4, aes(x = decile, y = average_enrichment)) +
...
#  geom_line(size=2, color='red') +
  ...
  labs(x = "Gene expression levels",
       y = "Mutation enrichment ratio \n (observed/expected)",
       title = "ID4")
ggsave(paste0(workdir,"ID4_enrichment_sc.pdf"), plot=p, height = 4, width = 5)
...
p <- ggplot(average_mut_num_summary_ID22, aes(x = decile, y = average_enrichment)) +
  geom_smooth(method=lm, se=T, size=1, colour="red") +
#  geom_line(size=2, color='red') +
  geom_point(size=2, colour="black") +
  scale_color_manual(values = cols) +
  ...
  labs(x = "Gene expression levels",
       y = "Mutation enrichment ratio \n (observed/expected)",
       title = "ID22")
ggsave(paste0(workdir,"ID22_enrichment_sc.pdf"), plot=p, height = 4, width = 5)
