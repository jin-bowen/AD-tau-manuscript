...
def scale_indel():
	...
	ID4_count = '/home/boj924/AD_Tau_PTA/results/ID83_Musical_AD/Assignment_Solution/Activities/Assignment_Solution_Activities.txt'
	burden_in = '/home/boj924/AD_Tau_PTA/results/tau_AD_ctrl_res.tab'
	...
	out_name    = '/home/boj924/AD_Tau_PTA/results/AT8-area_burden'
	...
	AT8pct['Case_ID'] = AT8pct['Case_ID'].astype(str)
	metafile['Case_ID'] = metafile['Case_ID'].astype(str)
...
#	var = 'ID22'
	...
	md = smf.ols(" %s ~ Age + AT8 + C(group, Treatment('noTau')) "%var, df_tau)
	mdf = md.fit()
	print(mdf.summary())


	md = smf.mixedlm(" %s ~ Age + AT8 + C(group, Treatment('noTau')) "%var, df_tau, groups=df_tau["Case_ID"])
	mdf = md.fit()
	rescale_results = mdf.summary().tables[1]
	rescale_results['Coef.'] = rescale_results['Coef.'].astype(float)
	...
	g=sns.lmplot(x="AT8", y=rescale_var, data=df_tau, hue='group', 
	...
	g=sns.lmplot(x="AT8", y=rescale_var, data=df_tau, \
